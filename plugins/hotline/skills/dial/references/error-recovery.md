...
- **The `not_found` degrade is now rare, not routine.** It used to fire on every dial from an agent-spawned session, because the opener let cmux scope the pane lookup to the inherited `$CMUX_WORKSPACE_ID`; it now pins `--workspace`/`--window` on every `new-surface` call, so a `not_found` means the pane or its workspace actually changed between the opener's tree snapshot and the call. If you see `surface-context→detached` on a dial today, read `surface_err.txt` — it is evidence of something moving, not the old inherited-workspace miss.
