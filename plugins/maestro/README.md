...
Decisions the human owns get a plain-language section in `/tmp/maestro/decisions-<session-id>.md`, written **before** the message that asks the question — what happened, why it matters, the options as the exact tokens to type, the recommendation, what happens next either way. A `Next for you:` that asks for a decision carries the answer tokens, a clause naming the subject, and a link to that section, because `still owe "fold" or "defer"` is unanswerable to a human who left four hours ago. Answered decisions move to a `Decided` list with the date and the answer, which is both the audit trail and the decisions half of a handoff.
